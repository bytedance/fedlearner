...
cdef class SGXChannelCredentials(ChannelCredentials):
  ...
  def __cinit__(self,
                const char* s_mr_enclave,
                const char* s_mr_signer,
    ...
    self.c_check_config = grpc_tls_server_authorization_check_config_create(
      NULL, sgx_server_auth_check_schedule, NULL, NULL)
...
def channel_credentials_sgxratls(mr_enclave, mr_signer, isv_prod_id, isv_svn):
  mr_enclave = bytes(mr_enclave, encoding = "ascii")
  mr_signer = bytes(mr_signer, encoding = "ascii")
  isv_prod_id = bytes(isv_prod_id, encoding = "ascii")
  isv_svn = bytes(isv_svn, encoding = "ascii")
  return SGXChannelCredentials(mr_enclave, mr_signer, isv_prod_id, isv_svn)


def get_sgx_server_key_cert():
  key_cert = _get_sgx_server_key_cert()
  return [(key_cert[0], key_cert[1])]
