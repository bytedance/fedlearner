...
int ra_tls_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    ret = sgx_qv_verify_quote((uint8_t*)quote, (uint32_t)quote_size, /*p_quote_collateral=*/NULL,
                              ...
                              supplemental_data);
    if (ret) {
        printf("sgx_qv_verify_quote error %d, %s\n", ret, mbedtls_high_level_strerr(ret));
    ...
    if (g_verify_measurements_cb) {
        ...
        printf("g_verify_measurements_cb failed, mre=%s, mrs=%s\n", (const char*)&quote->report_body.mr_enclave, (const char*)&quote->report_body.mr_signer);
    } else {
        /* use default logic to verify measurements */
        ret = verify_quote_against_envvar_measurements(quote, quote_size);
        printf("verify_quote_against_envvar_measurements failed, quote_size=%ld\n", quote_size);
    }
    if (ret < 0) {
        ret = MBEDTLS_ERR_X509_CERT_VERIFY_FAILED;
        goto out;
