...
class DataPortalJobManager(object):
    def __init__(self, mysql, portal_name, long_running):
        ...
        self._publisher = \
            RawDataPublisher(mysql, self._portal_manifest.raw_data_publish_dir)
    ...
    def _sync_portal_job(self, job_id):
        mysql_key = common.portal_job_mysql_key(self._portal_name, job_id)
    ...
    def _update_processing_job(self, job):
        self._processing_job = None
        mysql_key = common.portal_job_mysql_key(self._portal_name, job.job_id)
        self._mysql.set_data(mysql_key, text_format.MessageToString(job))
        self._processing_job = job

    def _sync_portal_manifest(self):
        if self._portal_manifest is None:
            mysql_key = common.portal_mysql_base_dir(self._portal_name)
            data = self._mysql.get_data(mysql_key)
            if data is not None:
    ...
    def _sync_job_part(self, job_id, partition_id):
        if partition_id not in self._job_part_map or \
            ...
            mysql_key = common.portal_job_part_mysql_key(self._portal_name,
                                                       job_id, partition_id)
            data = self._mysql.get_data(mysql_key)
            if data is None:
