...
class RawDataManager(visitor.IndexMetaManager):
    ...
    def _new_index_meta(self, process_index, start_index):
        ...
        if raw_data_meta.start_index == -1:
            ...
            mysql_key = common.raw_data_meta_mysql_key(
                    self._data_source.data_source_meta.name,
                    self._partition_id, process_index
                )
            if not self._mysql.cas(mysql_key, odata, ndata):
                raw_data_meta = self._sync_raw_data_meta(process_index)
    ...
    def _sync_raw_data_manifest(self):
        mysql_key = common.partition_manifest_mysql_key(
                self._data_source.data_source_meta.name,
                self._partition_id
            )
        data = self._mysql.get_data(mysql_key)
    ...
    def _preload_raw_data_meta(self):
        manifest_mysql_key = common.partition_manifest_mysql_key(
                self._data_source.data_source_meta.name,
                self._partition_id
            )
        all_metas = []
        index_metas = []
        for key, val in self._mysql.get_prefix_kvs(manifest_mysql_key, True):
...
class RawDataVisitor(visitor.Visitor):
    def __init__(self, mysql, data_source, partition_id, raw_data_options):
        super(RawDataVisitor, self).__init__(
                "raw_data_visitor",
                RawDataManager(mysql, data_source, partition_id)
            )
...
class FileBasedMockRawDataVisitor(RawDataVisitor):
    def __init__(self, mysql, raw_data_options,
        ...
        super(FileBasedMockRawDataVisitor, self).__init__(
                mysql, mock_data_source, 0, raw_data_options
            )

    def cleanup_meta_data(self):
        self._mock_rd_manifest_manager.cleanup_meta_data()

class MySQLBasedMockRawDataVisitor(RawDataVisitor):
    def __init__(self, mysql, raw_data_options, mock_data_source_name,
        ...
        self._mock_rd_manifest_manager = RawDataManifestManager(
                mysql, mock_data_source, False
            )
        self._partition_id = partition_id
        super(MySQLBasedMockRawDataVisitor, self).__init__(
                mysql, mock_data_source,
