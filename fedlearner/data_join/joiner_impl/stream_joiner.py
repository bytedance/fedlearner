...
class StreamExampleJoiner(ExampleJoiner):
    def __init__(self, example_joiner_options, raw_data_options,
        ...
        self._example_with_same_id_buffer = {}
    ...
    def _update_join_cache(self):
        ...
        for example_id in self._leader_unjoined_example_ids:
            if example_id in self._follower_example_cache:
                """ if multiple examples are generated by a same example id, and example_id_allow_dedup is enabled,
                    cache it in _example_with_same_id_buffer
                """
                if self._example_joiner_options.example_id_allow_dedup and \
                   example_id in self._joined_cache:
                    if example_id not in self._example_with_same_id_buffer:
                        self._example_with_same_id_buffer[example_id] = []
                    self._example_with_same_id_buffer[example_id]\
                            .append(self._follower_example_cache[example_id]);
    ...
    def _dump_joined_items(self):
        ...
        for (li, le) in self._leader_join_window:
            ...
            for fi,item in self._example_with_same_id_buffer[eid]:
    ...
    def _finish_data_block(self):
        ...
        for index, item in self._joined_cache.values():
            if index < self._follower_restart_index:
                self._follower_restart_index = index
            if item.example_id in self._example_with_same_id_buffer:
                for index, _ in self._example_with_same_id_buffer[item.example_id]:
                    if index < self._follower_restart_index:
                        self._follower_restart_index = index
        return meta
