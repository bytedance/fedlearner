...
class RawDataManifestManager(object):
    def __init__(self, mysql, data_source, batch_mode=False):
    ...
    def cleanup_meta_data(self):
        with self._lock:
            ...
            mysql_base_key = common.data_source_mysql_base_dir(data_source_name)
            self._mysql.delete_prefix(mysql_base_key)
    ...
    def _try_to_sub_raw_data(self, partition_id):
        ...
        while True:
            ...
            pub_data = self._mysql.get_data(mysql_key)
            if pub_data is None:
                break
    ...
    def _store_raw_data_metas(self, partition_id, new_raw_data_metas):
        if len(new_raw_data_metas) > 0:
            ...
            for raw_date_meta in new_raw_data_metas:
                ...
                self._mysql.set_data(mysql_key, data)
    ...
    def _get_manifest(self, partition_id):
        ...
        manifest_data = self._mysql.get_data(manifest_mysql_key)
    ...
    def _update_manifest(self, manifest):
        ...
        self._local_manifest[partition_id] = None
        self._mysql.set_data(manifest_mysql_key,
    ...
    def _process_next_process_index(self, partition_id, manifest):
        ...
        while True:
            meta_mysql_key = \
                    common.raw_data_meta_mysql_key(
                            self._data_source.data_source_meta.name,
                            partition_id, next_process_index
                        )
            data = self._mysql.get_data(meta_mysql_key)
            if data is None:
                break
