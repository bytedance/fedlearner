...
def upgrade():
    ...
    op.alter_column('job_v2', 'state', nullable=False, comment='state', type_=sa.Enum('INVALID', 'STOPPED', 'WAITING', 'STARTED', 'NEW', 'COMPLETED', 'FAILED', name='jobstate', native_enum=False, create_constraint=False))
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    bind = op.get_bind()
    op.alter_column('job_v2', 'state', nullable=False, comment='state', type_=sa.Enum('INVALID', 'STOPPED', 'WAITING', 'STARTED', name='jobstate', native_enum=False, create_constraint=True))
    # ### end Alembic commands ###
