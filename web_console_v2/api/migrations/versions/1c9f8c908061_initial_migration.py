...
def upgrade():
    ...
    op.create_table('federations_v2',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('name', sa.String(length=255), nullable=True),
    sa.Column('config', sa.Text(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_federations_v2_name'), 'federations_v2', ['name'], unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_federations_v2_name'), table_name='federations_v2')
    op.drop_table('federations_v2')
    # ### end Alembic commands ###
