...
def rison_postprocess(rison_str):
    ...
    return pattern.sub(
        lambda match: RISON_REPLACEMENT[match.group(0)],
        rison_str
    )
...
def create_rate_visualization(job, args):
    ...
    """
    vis_state = _vis_state_preprocess(job, args)
...
def create_numeric_visualization(job, args):
    ...
    for k in ('aggregator', 'value_field', 'metric_name'):
        if not (k in args and args[k] is not None):
            raise ValueError('[{}] should be provided in Numeric visualization.'
                             .format(k))
    assert args['aggregator'] in AGG_TYPE_MAP
    vis_state = _vis_state_preprocess(job, args)
    params = vis_state['params']
    ...
    series = _create_series(
        **{'label': '{} of {}'.format(args['aggregator'],
                                      args['metric_name']),
           'metrics': {'type': AGG_TYPE_MAP[args['aggregator']],
