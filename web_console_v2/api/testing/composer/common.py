...
class TestRunner(IRunnerV2):

    def __init__(self, with_exception=False):
        self._with_exception = with_exception
        self.context = None

    def run(self, context: RunnerContext) -> Tuple[RunnerStatus, RunnerOutput]:
        logging.info(f'[test_runnner] context: {context}')
        self.context = context
        pp_time.sleep(2)
        if self._with_exception:
            raise RuntimeError('fake error')
        if context.index == 3:
            return RunnerStatus.FAILED, RunnerOutput(error_message='index is 3')
