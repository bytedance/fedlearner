    ...
    while True:
        if i % 3 == ordering[0]:  # fix lam20
            ...
            def f(x): return symKL_objective(lam10=D - (d - 1) * x,
                                             lam20=x, lam11=lam11, lam21=LAM21, u=u, v=v, d=d, g=g)
            # f_prime = lambda x: (d-1)/v - (d-1)/(lam11+v) - (d-1)*v/((x+u)**2) + (lam11 + v + g)*(d-1)/((D-(d-1)*x+u)**2)

            def f_prime(x): return (d - 1) / v - (d - 1) / (lam11 + v) - (d - 1) / (x + u) * (v / (
                x + u)) + (lam11 + v + g) / (D - (d - 1) * x + u) * ((d - 1) / (D - (d - 1) * x + u))
            # print('D/d', D/d)
            ...
            def f(x): return symKL_objective(lam10=lam10, lam20=x, lam11=D / \
                  p - (1 - p) * (d - 1) * x / p, lam21=LAM21, u=u, v=v, d=d, g=g)
            # f_prime = lambda x: (d-1)/v - (1-p)*(d-1)/(lam10 + u)/p - (d-1)*v/((x+u)**2) + (lam10+u+g)*(1-p)*(d-1)/p/((D/p - (1-p)*(d-1)*x/p + v)**2)
            def f_prime(x): return (d - 1) / v - (1 - p) * (d - 1) / (lam10 + u) / p - (d - 1) / (x + u) * (v / (x + u)) + (
                lam10 + u + g) / (D / p - (1 - p) * (d - 1) * x / p + v) * (1 - p) * (d - 1) / p / (D / p - (1 - p) * (d - 1) * x / p + v)
    ...
    while True:
        if i % 3 == ordering[0]:  # fix lam21
            ...
            def f(x): return symKL_objective(lam10=(D - p * (d - 1) * x) / \
                  (1 - p), lam20=LAM20, lam11=lam11, lam21=x, u=u, v=v, d=d, g=g)
            # f_prime = lambda x: (d-1)/u - p*(d-1)/(lam11+v)/(1-p) - (d-1)*u/((x+v)**2) + (lam11 + v + g)*p*(d-1)/(1-p)/(((D - p*(d-1)*x)/(1-p) + u)**2)

            def f_prime(x): return (d - 1) / u - p * (d - 1) / (lam11 + v) / (1 - p) - (d - 1) / (x + v) * (u / (x + v)) + (
                lam11 + v + g) / ((D - p * (d - 1) * x) / (1 - p) + u) * p * (d - 1) / (1 - p) / ((D - p * (d - 1) * x) / (1 - p) + u)
            ...
            def f(x): return symKL_objective(lam10=lam10, lam20=LAM20,
                                             lam11=D - (d - 1) * x, lam21=x, u=u, v=v, d=d, g=g)
            # f_prime = lambda x: (d-1)/u - (d-1)/(lam10+u) - (d-1)*u/((x+v)**2) + (lam10 + u + g)*(d-1)/((D-(d-1)*x+v)**2)
            def f_prime(x): return (d - 1) / u - (d - 1) / (lam10 + u) - (d - 1) / (x + v) * (
                u / (x + v)) + (lam10 + u + g) / (D - (d - 1) * x + v) * (d - 1) / (D - (d - 1) * x + v)
            # def f_prime(x):
            #     print('x', x)
...
if __name__ == '__main__':
    ...
    # print(zero_uv_problem_string(g=g, p=p, P=P))

    # minimize 99*(z + 1)/2 + 99*2/(z+1)+(x+1+10)/(y+2) + (y+2+10)/(x+1)
    # subject to x>=0, y>=0, z>=0, z<=x, 0.1*y+0.9*x+0.9*99*z=10
    ...
    #             lam20 = random.random() * P / (1-p) / d
    #             # print('lam21', lam21)
    #             solutions.append(solve_small_neg(u=u,v=v,d=d,g=g,p=p,P=P, lam10=0.0, lam11=0.0, lam20=lam20))
    #         elif j % 3 == 1:
    #             lam11 = random.random() * P / p
    #             # print('lam11', lam11)
    #             solutions.append(solve_small_neg(u=u,v=v,d=d,g=g,p=p,P=P, lam10=0.0, lam11=lam11, lam20=0.0))
    #         else:
    #             lam10 = random.random() * P / (1-p)
    #             # print('lam10', lam10)
    #             solutions.append(solve_small_neg(u=u,v=v,d=d,g=g,p=p,P=P, lam10=lam10, lam11=0.0, lam20=0.0))
    #     print('time taken', time.time() - start)

    #     counter = Counter()
    #     counter.update([solution[-1] for solution in solutions])
    #     print(counter)
    #     print(small_neg_problem_string(u=u,v=v,d=d,g=g,p=p,P=P))
    ...
    #             lam21 = random.random() * P / p / d
    #             # print('lam21', lam21)
    #             solutions.append(solve_small_pos(u=u,v=v,d=d,g=g,p=p,P=P, lam10=0.0, lam11=0.0, lam21=lam21))
    #         elif j % 3 == 1:
    #             lam11 = random.random() * P / p
    #             # print('lam11', lam11)
    #             solutions.append(solve_small_pos(u=u,v=v,d=d,g=g,p=p,P=P, lam10=0.0, lam11=lam11, lam21=0.0))
    #         else:
    #             lam10 = random.random() * P / (1-p)
    #             # print('lam10', lam10)
    #             solutions.append(solve_small_pos(u=u,v=v,d=d,g=g,p=p,P=P, lam10=lam10, lam11=0.0, lam21=0.0))
