...
def convert_dp_sensitivity_to_std(sensitivity, epsilon, delta, mechanism="Gaussian"):
...
def generate_noisy_value_w_dp(sensititvity = 1.0, noise_eps=0.0, noisy_mechanism="Gaussian"):
    noise_std = convert_dp_sensitivity_to_std(sensititvity, noise_eps, delta=0.0, mechanism=noisy_mechanism)
    if noise_std > 0.0:
...
class DataSample:
    ...
    def flip_label(self, dp_epsilon = None, flipping_prob = None):
        ...
        self.report_label = output * (1 - self.true_label) + (1 - output) * self.true_label
...
class Client:
    ...
    def aggregate_quadruples(self, threshold):
        ...
        if run_parallel:
            ...
            result = pool.starmap(report_quadruple_per_sample, [(a_sample, threshold) for a_sample in self.data_samples])
            pool.close()
            pool.join()
            # print("results: {}".format(result))
            return reduce(lambda x,y: x+y, result)
        else:
            return reduce(lambda x,y: x+y, map(lambda x: np.array(x.report_quadruple(threshold)), self.data_samples))

    def report_aggregated_quadruples(self, threshold, dp_noise_mechanism="None", dp_noise_eps=0.0):
        vanilla_res = self.aggregate_quadruples(threshold)
        # print("vanilla_res_quadruples: {}".format(vanilla_res))
        noise = np.array([generate_noisy_value_w_dp(sensititvity = 1.0, noise_eps=dp_noise_eps, noisy_mechanism=dp_noise_mechanism)
                                                for _ in range(4)])
...
class DataSet:
    def __init__(self, clients):
        ...
        self.all_data_samples = list(chain.from_iterable([client.data_samples for client in self.clients]))
    ...
    def cal_quadruples_once(self, sampled_clients_ratio = 1.0, threshold = 0.5, dp_noise_mechanism="None", dp_noise_eps=0.0):
        # print(type(self.clients))
        sampled_clients = random.sample(self.clients, int(self.number_of_clients * sampled_clients_ratio))
        # print("sampled_clients: {}, corresponding length: {}".format(sampled_clients, len(sampled_clients)))
        
        def report_aggregated_quadruples_per_client(a_client, threshold, dp_noise_mechanism, dp_noise_eps):
            return a_client.report_aggregated_quadruples(threshold, dp_noise_mechanism=dp_noise_mechanism, dp_noise_eps=dp_noise_eps)

        if run_parallel:
            pool = Pool(processes=nprocs)
            result = pool.starmap(report_aggregated_quadruples_per_client,
            ...
            res = reduce(lambda x, y: x + y,
                    map(lambda x: x.report_aggregated_quadruples(threshold,
                            dp_noise_mechanism=dp_noise_mechanism, dp_noise_eps=dp_noise_eps),
                                sampled_clients))
        ...
        tp, fp, tn, fn = max(res[0],0), max(res[1],0), max(res[2],0), max(res[3],0)
    ...
    def cal_fpr_tpr(self, sampled_clients_ratio = 1.0, thresholds = [0.5], dp_noise_mechanism="None", dp_noise_eps=0.0):
        if run_parallel:
            pool = Pool(processes=nprocs)
            res = pool.starmap(self.cal_quadruples_once,
                     [(sampled_clients_ratio, threshold, dp_noise_mechanism, dp_noise_eps) for threshold in thresholds])
            pool.close()
            pool.join()
        else:
            res = [self.cal_quadruples_once(sampled_clients_ratio, threshold, dp_noise_mechanism, dp_noise_eps) for threshold in thresholds]
    ...
    def cal_roc_auc(self, sampled_clients_ratio = 1.0, thresholds = [0.5], dp_noise_mechanism="None", dp_noise_eps=0.0):
        fprs, tprs = self.cal_fpr_tpr(sampled_clients_ratio, thresholds, dp_noise_mechanism, dp_noise_eps)
    ...
    def aggregate_noisy_number_of_positives(self):
        ...
        self.total_noisy_number_of_positives = sum([client.report_noisy_number_of_positives() for client in self.clients])
        return self.total_noisy_number_of_positives
    def aggregate_noisy_number_of_negatives(self):
        # based on randomized responses
        self.total_noisy_number_of_negatives = sum([client.report_noisy_number_of_negatives() for client in self.clients])
        return self.total_noisy_number_of_negatives

    def aggregate_true_number_of_positives(self):
        self.total_true_number_of_positives = sum([client.true_number_of_positives for client in self.clients])
        return self.total_true_number_of_positives

    def aggregate_true_number_of_negatives(self):
        self.total_true_number_of_negatives = sum([client.true_number_of_negatives for client in self.clients])
    ...
    def convert_corrupted_auc_to_real(self, p2n_p, n2p_q, auc_corrupted):
        true_total_number_of_positives = sum([client.true_number_of_positives for client in self.clients])
        true_total_number_of_negatives = sum([client.true_number_of_negatives for client in self.clients])
        base_rate = true_total_number_of_positives * 1.0 / (true_total_number_of_positives + true_total_number_of_negatives) # pi
        print("true base_rate: {}".format(base_rate))
        estimated_total_number_of_positives =((self.total_noisy_number_of_positives * (1.0 - n2p_q) - self.total_noisy_number_of_negatives * n2p_q)
                                               /(1.0 - p2n_p - n2p_q) )
        estimated_total_number_of_negatives = true_total_number_of_positives + true_total_number_of_negatives - estimated_total_number_of_positives
        base_rate = estimated_total_number_of_positives * 1.0 / (estimated_total_number_of_negatives + estimated_total_number_of_positives)
        print("base_rate_corr: {}".format(base_rate))
        alpha = (1.0 - base_rate) * n2p_q / (base_rate * (1.0 - p2n_p) + (1.0 - base_rate) * n2p_q)
        beta = base_rate * p2n_p / (base_rate * p2n_p + (1.0 - base_rate) * (1.0 - n2p_q))
        # print("base_rate: {}, alpha: {}, beta: {}".format(base_rate, alpha, beta))
        auc_real = (auc_corrupted - (alpha + beta) / 2.0) / (1.0 - alpha - beta)
        return auc_real

    def cal_ROC_AUC_rr(self, label_flipping_eps = 10000.0, sampled_clients_ratio = 1.0, thresholds = [0.5]):

        label_flipping_prob = dp_flipping_rate(label_flipping_eps)
        self.flip_all_labels(label_flipping_prob)
        auc_corrupted = self.cal_roc_auc(sampled_clients_ratio = sampled_clients_ratio,
                                         thresholds = thresholds,
                                         dp_noise_mechanism="None",
                                         dp_noise_eps=label_flipping_eps)

        auc_real = self.convert_corrupted_auc_to_real(p2n_p = label_flipping_prob, n2p_q= label_flipping_prob, auc_corrupted=auc_corrupted)
        print("auc_corrupted: {}, auc_real: {}".format(auc_corrupted, auc_real))
        return auc_real

    def report_final_ROC_AUC(self, sampled_clients_ratio = 1.0, thresholds = [0.5], dp_noise_mechanism="RR", dp_noise_eps=10000.0):
        if dp_noise_mechanism == "RR" or dp_noise_mechanism == "rr":
            return self.cal_ROC_AUC_rr(label_flipping_eps=dp_noise_eps, sampled_clients_ratio=sampled_clients_ratio, thresholds=thresholds)
        else:
            noisy_auc = self.cal_roc_auc(sampled_clients_ratio = sampled_clients_ratio,
                                         thresholds = thresholds,
                                         dp_noise_mechanism=dp_noise_mechanism,
                                         dp_noise_eps=dp_noise_eps)
            print("noisy_auc:{} with: {} and eps: {}".format(noisy_auc, dp_noise_mechanism, dp_noise_eps ))
            return noisy_auc
...
def expected_std_auc_1(n, p, c, epsilon, delta):
    if epsilon > 0:
        return (n + p - 1.0) * np.sqrt(2 * np.log(1.25/delta)) / epsilon / (p * n / np.sqrt(c))
    else:
        return -1.0
...
def expected_std_auc_rr(n, p, c, epsilon):
    ...
    return np.sqrt(prob * (1.0 - prob) * c * (c + 1.0)* (2*c + 1.0) / 6.0)/ (p * n)
...
if __name__ == "__main__":
    ...
    data = [(0.8, 0), (0.1, 0), (0.2, 0), (0.4, 0), (0.5, 0), (0.6, 1), (0.7, 1), (0.88, 0), (0.12, 1), (0.98,1), (0.98, 1), (0.998, 0), (0.765, 1), (0.892, 0), (0.34, 1)]
    ...
    for sample in dataset.all_data_samples:
        print("client:{}, score: {}, report label: {}, ranking: {}".format(sample.client_id, sample.prediction_score, sample.report_label, sample.report_quadruple(threshold)))
