...
dp_noise_eps_list = ["0.01"] # total epsilon = dp_noise_eps * num_thresholds * 4 for Laplace; For RR, it's the dp budget as it is. 
    ...
    for j, noise_eps in enumerate(dp_noise_eps_list):
        for k, num_threshold in enumerate(num_thresholds):
            for z, dp_noise_mechanism in enumerate(dp_noise_mechanisms):
                dp_noise_mechanism_opt = ["--dp_noise_mechanism", str(dp_noise_mechanism)]
                ...
                args_list = (common_options + number_clients_opt + dp_noise_mechanism_opt
                                    + dp_noise_eps_opt + repeat_times_opt + num_thresholds_opt)
                if using_gpu:
                    idx = i * (len(dp_noise_eps_list) * len(num_thresholds)) + j * len(num_thresholds) + k
                    args_list += gpu_opt + gpus[(gpu_start_idx + idx)%8]
                log_file_name = "logs/stdout_logs/" + str(stamp) + "_numberClients_" + number_clients + str(dp_noise_mechanism) + "_eps_" + noise_eps
                log_file_name += "numThreshold_" + str(num_threshold) + "_repeatTimes_" + str(repeat_times)
                if assign_client_id_ranking_skewed:
                    assign_client_id_ranking_skewed_opt = ["--clients_id_assigned_ranking_skewed"]
