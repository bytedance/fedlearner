...
parser.add_argument("--clients_sampled_ratio", type=float, default=1.0, help='Clients sampled ratio when selecting clients to cal ROC AUC')
parser.add_argument('--one_sample_per_device', action='store_true', help='one sample per device')
parser.add_argument('--num_thresholds', type=int, default=100) # AUC calculattion thresholds
parser.add_argument('--repeat_times', type=int, default=1)  # repeat the calcuation many times to get the mean and std of the estimated AUC
parser.add_argument('--is_full_dataset', action='store_true', help='using the full dataset to evaluate')
parser.add_argument('--clients_id_assigned_ranking_skewed', action='store_true', help='if ture: clients_id_assigned_ranking_skewed, otherwise assign clients id uniformly')

parser.add_argument("--dp_noise_epsilon", type=float, default=0.0, help='dp noise epsilon for each value in the quadruple (i.e. TP). The total dp budget is: 4 * num_thresholds * dp_noise_epsilon')
parser.add_argument('--dp_noise_mechanism', type=str, default="None", help = "Gaussian, Laplace, RR (rr), None")
...
def assign_client_id_uniformly(sample_id,number_clients,max_client_id,one_sample_per_device=False):
    if one_sample_per_device or number_clients >= max_client_id:
        return sample_id
    else:
        return np.random.randint(0, high=number_clients)

def assign_client_id_ranking_skewed(ranking_idx,number_clients,max_client_id):
...
def load_dataset(is_full = False):
    if is_full:
        ...
        label_pred_dict = pickle.load(open("./data/test_0.1_label_pred.pkl", "rb"))
...
def multi_epoch_run(is_full = False):
    ...
    for epoch, d in label_pred_dict.items():
        ...
        auc_roc_list = list(map(lambda x: cal_auc_one_time(), range(repeat_times)))
