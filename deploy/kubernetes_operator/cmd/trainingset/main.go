...
func main() {
	...
	if err := v1alpha2.AddToScheme(scheme.Scheme); err != nil {
		klog.Fatalf("Failed to add trainingset scheme: %v", err)
	}
	...
	recorder := eventBroadcaster.NewRecorder(scheme.Scheme, apiv1.EventSource{Component: "trainingset-operator"})
	...
	trainingSetController := trainingset.NewController(kubeClient, crdClient, kubeInformerFactory, crdInformerFactory)
	...
	if err := trainingSetController.Run(*workerNum, stopCh); err != nil {
		klog.Fatal(err)
	}
